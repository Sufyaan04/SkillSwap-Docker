        ...
        <div className="overflow-hidden rounded-2xl shadow-xl border border-border bg-background flex flex-col md:flex-row">
          ...
          <div className="flex flex-col gap-6 p-8 md:p-10 flex-1">
            ...
            <div className="flex flex-col gap-1">
              ...
              <p className="text-sm text-muted-foreground">Login to your SkillSwap account</p>
            </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              {/* Social buttons — UI only, no OAuth needed for now */}
              <div className="grid grid-cols-3 gap-2">
                {["Apple", "Google", "GitHub"].map((provider) => (
                  <button
                    ...
                    className="flex h-10 items-center justify-center rounded-md border border-input bg-background px-4 text-xs font-medium text-foreground transition-colors hover:bg-accent hover:text-accent-foreground"
                  >
                    {provider === "Google" && (
          ...
          <div className="relative hidden md:flex flex-1 flex-col justify-between bg-zinc-900 p-10 text-white overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="mt-3 text-sm text-zinc-400 leading-relaxed max-w-xs">
                The platform where students and professionals exchange expertise — no money, just knowledge.
              </p>
            </div>

            {/* Middle — floating exchange cards */}
            <div className="relative z-10 flex flex-col gap-3 my-6">
              {[
                { from: "React Dev", to: "UI Design", color: "#00e5a0" },
                { from: "Python", to: "Data Science", color: "#6c8cff" },
                { from: "Guitar", to: "Music Theory", color: "#ff9f47" },
              ].map((item, i) => (
                <div key={i} className="flex items-center gap-3 rounded-xl bg-white/10 backdrop-blur px-4 py-3 text-sm border border-white/10">
                  <span className="font-semibold" style={{ color: item.color }}>{item.from}</span>
            ...
            <div className="relative z-10 flex gap-6">
              <div>
                ...
                <div className="text-xs text-zinc-500 mt-0.5">Active swappers</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-emerald-400">50+</div>
                <div className="text-xs text-zinc-500 mt-0.5">Skills available</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-emerald-400">100%</div>
                <div className="text-xs text-zinc-500 mt-0.5">Free forever</div>
              </div>
            </div>
          </div>
